package com.yunk.facedetectionandroid;
...
public class AsyncDatabaseTrainer extends AsyncTask<String, String, String> {
	...
	/*	
		try {
			...
			System.out.println("yunk first pass done " + assetFiles.length);
			for(int f = 0 ; f <= (assetFiles.length-1); f++)
			{
				System.out.println("yunk first pass " + assetFiles[f]);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}*/
		...
		try {
			...
			for(int f = 0; f <= (temp.size()-1);f++){
				System.out.println("yunk thw " + temp.get(f) );
			}
			...
			System.out.println("yunk thw pass done");
			//assetFiles = context.getFilesDir().list();						//files names use as labels later on
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if(assetFiles.length == 0)
		{
			...
			try {
				...
				System.out.println("yunk second pass done");
			} catch (Exception e) {
				System.out.println("yunk the error faced was " + e.getMessage());
			}
		...
		System.out.println("yunk new bug " + "Starting to load algorithm");
		...
		else if(MainActivity.algorithm.equals("Fisher"))
		{
			...
			MainActivity.faceRecognizer = createEigenFaceRecognizer(FisherComponents,
					FisherThreshold);
			System.out.println("yunk current is hola " + FisherThreshold + " comp " + FisherComponents);
		}
			...
			MainActivity.algorithm = "Eigen";
			System.out.println("yunk current is hola " + EigenThreshold + " comp " + EigenComponents);
	...
	private String readMemory(String alg)
	 {
		 ...
		 {
			 System.out.println("yunk error " + e.getMessage());
		 }
		 return null;
	 }
}
